...
router.post('/updatebodystatus', [
    // check if head exists
    check('head').isLength({ min: 1 }).withMessage('Head can\'t be blank.'),
    // check if arms exists
    check('arms').isLength({ min: 1 }).withMessage('Arms can\'t be blank.'),
    // check if legs exists
    check('legs').isLength({ min: 1 }).withMessage('Legs can\'t be blank.'),
], (req, res, next) => {
